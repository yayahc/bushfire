...
def allumage(siteEnFeu,cotéAllumé,longueurEclosion,nbEclosion,coorx,coory,\
             ...
             directionDuVent,deltax,deltay,veg,temperatureAllumage, temperatureSite):
    import numpy as np
...
#################programme principal###################
def temperature_site(timp,typeVegetation,x1veg,x2veg,y1veg,y2veg,\
                     teneurEnEau,hauteurFuelBed,emissiviteFuelBed,absorptionFuelBed,\
                     densitéFuelBed,diametreFuelBed,chaleurSpecFeulBed,chargeSurface,\
                     hauteurFlamme,timeCombustion,timeMax,deltat,deltax,deltay,\
                     xDebut,xFin,yDebut,yFin,temperatureAir,vitesseDuVent,directionDuVent,\
                     pentex,pentey,temperatureFlamme,temperatureAllumage,temperatureBraise,\
                     emissiviteBraise,conductiviteBraise,conductiviteFlamme,viscositeAirChaud,\
                     nbEclosion,xEclosion,yEclosion,timeAllumage,longueurEclosion,xenreg,\
                     yenreg):
...
##timp=float(input("entrez la fréquence d'impression en seconde:"))
##print("lecture fichier des données")
##typeVegetation=int(input("indiquer le nombre de classe de vegetation ="))
...
#
    ...
    siteVoisin=[]   
    siteBrule=[]
    file=r"D:\Code_RosFire_Python\FichierDeDonnees.xlsx" #specifier le chemin du fichier de donnees
    vegetation=pd.read_excel(file,sheet_name=0,usecols=np.arange(typeVegetation+1))   #,usecols=(0,1)
##print(vegetation)
    veget1=vegetation['vegetation1'].to_numpy()  #choix de la colonne pour lecture
...
#lecture de la second feuille de donnees
    climat=pd.read_excel(file,sheet_name=1,usecols=(0,1))    
##print(climat)
    clim=climat['donnees1'].to_numpy()    
...
##nbEclosion=int(input("le nombre de depart de feu ="))
    nbreDepartFeu=nbEclosion
    eclosion=pd.read_excel(file,sheet_name=2,usecols=np.arange(nbEclosion+1))
##print(eclosion)
    eclos1=eclosion['depart1'].to_numpy()
